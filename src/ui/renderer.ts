...
const TEXT_COLORS: { [value: number]: string } = {
  2: '#776e65',
  4: '#776e65',
  6: '#f9f6f2',
  8: '#f9f6f2',
...
export class Renderer {
  ...
  private calculateLayout(boardSize: number): void {
  ...
  // Draw board background
  private drawBoardBackground(boardSize: number): void {
    const boardWidth = this.tileSize * boardSize + this.padding * (boardSize + 1);
    const boardHeight = this.tileSize * boardSize + this.padding * (boardSize + 1);
  ...
  // Draw a single tile
  private drawTile(tile: Tile, row: number, col: number, boardSize: number): void {
    const x = this.boardOffsetX + this.padding + col * (this.tileSize + this.padding);
    const y = this.boardOffsetY + this.padding + row * (this.tileSize + this.padding);
  ...
  // Draw goal progress
  private drawGoalProgress(state: GameState): void {
    ...
    this.ctx.textAlign = 'center';
    this.ctx.fillText(
      `目标: ${state.level.goal.description}`,
      this.width / 2,
      y
    );
  ...
  // Draw moves remaining
  private drawMovesRemaining(state: GameState): void {
    ...
    this.ctx.textAlign = 'center';
    this.ctx.fillText(
      `剩余步数: ${state.maxMoves - state.moves}`,
      this.width / 2,
      y
    );
  }
  ...
  private drawGameOverOverlay(state: GameState): void {
  ...
  // Draw a button
  private drawButton(text: string, x: number, y: number, width: number, height: number): void {
    // Button background
